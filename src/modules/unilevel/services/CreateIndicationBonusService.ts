...
class CreateIndicationBonusService {
  ...
  public async execute({
    ...
    const directNode = await this.unilevelNodesRepository.findByUserId(direct_id);
    if (!directNode) return result;

    const indirect1_id = directNode.indicator_id;
    const indirect1Count = await this.unilevelNodesRepository.findAllIndicateds(indirect1_id);    
    ...
    result.push({
      ...
      usd_cents: new BigNumber(usd_cents).multipliedBy(0.02).toNumber(),
    });

    const indirect1Node = await this.unilevelNodesRepository.findByUserId(indirect1_id);
    if (!indirect1Node) return result;

    const indirect2_id = indirect1Node.indicator_id;
    const indirect2Count = await this.unilevelNodesRepository.findAllIndicateds(indirect2_id);
    ...
    const indirect2Node = await this.unilevelNodesRepository.findByUserId(indirect2_id);
    if (!indirect2Node) return result;

    const indirect3_id = indirect2Node.indicator_id;
    const indirect3Count = await this.unilevelNodesRepository.findAllIndicateds(indirect3_id);
    ...
    const indirect3Node = await this.unilevelNodesRepository.findByUserId(indirect3_id);
    if (!indirect3Node) return result;

    const indirect4_id = indirect3Node.indicator_id;
    const indirect4Count = await this.unilevelNodesRepository.findAllIndicateds(indirect4_id);
    ...
    const indirect4Node = await this.unilevelNodesRepository.findByUserId(indirect4_id);
    if (!indirect4Node) return result;

    const indirect5_id = indirect4Node.indicator_id;
    const indirect5Count = await this.unilevelNodesRepository.findAllIndicateds(indirect5_id);
    ...
    const indirect5Node = await this.unilevelNodesRepository.findByUserId(indirect5_id);
    if (!indirect5Node) return result;

    const indirect6_id = indirect5Node.indicator_id;
    const indirect6Count = await this.unilevelNodesRepository.findAllIndicateds(indirect6_id);
    ...
    result.push({
      ...
      description: 'Indirect Indication Bonus',
      usd_cents: new BigNumber(usd_cents).multipliedBy(0.01).toNumber(),
    });

    const indirect6Node = await this.unilevelNodesRepository.findByUserId(indirect6_id);
    if (!indirect6Node) return result;

    const indirect7_id = indirect6Node.indicator_id;
    const indirect7Count = await this.unilevelNodesRepository.findAllIndicateds(indirect7_id);
